...
1. this package's Babel transform (`transformReactAlienSignals`), in the mode under test;
...
The emitted code is byte-for-byte the transform's own output, and the runtime test updates correctly. This is a bail-out on unsupported syntax, not a compatibility guarantee: it depends on the compiler continuing to reject `try` without `catch`, and with `panicThreshold: "all_errors"` the same event fails the build instead of skipping the function. The opt-out directive is therefore emitted in managed mode too.
...
- **`panicThreshold: "all_errors"` with `transform: "managed"`.** The directive does not suppress the `TryStatement` error event — it is still logged with the directive present — so a build configured to panic on all errors is expected to fail on managed output. Not reproduced end-to-end in a real bundler.
- **Hand-written `useSignals()` without the build plugin.** Nothing inserts the directive there, and the failure is silent. Such components need `"use no memo"` written by hand, or the plugin in `mode: "manual"`, which now adds it for them.
