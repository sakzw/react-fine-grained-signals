...
- pluginの既存の挙動(scope/binding awareな `useSignalTracking` 検出、nested boundaryのskipロジック、comment directiveの処理、`memo`/`forwardRef` のwrapper検出)に対する完全なcorrectness parityを維持する。
